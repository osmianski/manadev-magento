 ...
 *
 */
class Mana_Seo_Resource_UrlIndexer_CategoryValue extends Mana_Seo_Resource_UrlIndexer {
    /**
    ...
    public function process($indexer, $schema, $options) {
        ...
        $select = $db->select()
            ...
            ->joinLeft(array('ug' => $this->_resources->getTableName($urlKeyAttributeTable)),
                "`ug`.`entity_id` = `e`.`entity_id`".
                $db->quoteInto(" AND `ug`.`attribute_id` = ?", $urlKeyAttribute['attribute_id']).
                " AND `ug`.`store_id` = 0", null)
            ->joinLeft(array('us' => $this->_resources->getTableName($urlKeyAttributeTable)),
            ...
            ->joinLeft(array('ag' => $this->_resources->getTableName($isActiveAttributeTable)),
                "`ag`.`entity_id` = `e`.`entity_id`" .
                $db->quoteInto(" AND `ag`.`attribute_id` = ?", $isActiveAttribute['attribute_id']) .
                " AND `ag`.`store_id` = 0", null)
            ->joinLeft(array('as' => $this->_resources->getTableName($isActiveAttributeTable)),
